//----====|| Angular App ||====----\\
var app = angular.module('Demo', ['ngMaterial','ngRecordQue']).config(function($mdThemingProvider){ $mdThemingProvider.theme('default').dark(); });
...
var arrIcons = [
	...
	$scope.fnAddRecordQueRule=function(){
		console.log($scope.tmp.shape,$scope.tmp.color);
		$scope.tmp.recordQue[$scope.tmp.region].push({'region':$scope.tmp.region,'path':$scope.tmp.path,'value':$scope.tmp.value,'op':$scope.tmp.op,'class':[$scope.tmp.shape,$scope.tmp.color].join(' ')});
		console.log($scope.tmp.recordQue);
	}
